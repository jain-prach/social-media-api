...
def test_delete_post_for_no_post_created(before_create_public_user_login_cred):
    ...
    token = before_create_public_user_login_cred(session=session)
    session.close()
    response = client.delete(f"/post/{uuid.uuid4()}/", headers=get_auth_header(token=token))
    assert response.status_code == 200

def test_delete_post_of_another_user(before_create_public_user_login_cred, before_create_post):
    session = create_session()
    token = before_create_public_user_login_cred(session=session)
    post = before_create_post(
        session=session,
