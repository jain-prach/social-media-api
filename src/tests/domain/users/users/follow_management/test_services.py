...
from src.tests.test_fixtures import before_create_follow_request, before_create_approved_follow_requests, before_create_base_user, before_create_normal_user
...
def test_get_follow_by_follow_id(before_create_normal_user, before_create_follow_request):
    ...
    db_follow = before_create_follow_request(session=session, follower_id=user1.id, following_id=user2.id)
...
def test_get_follow_for_follower_and_following(before_create_normal_user, before_create_follow_request):
    session = create_session()
    user1 = before_create_normal_user(session=session, user_dict=create_private_user())
    user2 = before_create_normal_user(session=session, user_dict=create_private_user())
    db_follow = before_create_follow_request(session=session, follower_id=user1.id, following_id=user2.id)
    follow = FollowService(session=session).get_follow_for_follower_and_following(follower_id=user1.id, following_id=user2.id)
    assert db_follow == follow
    session.close()

def test_get_follow_for_follower_and_following_for_no_user():
    session = create_session()
    follow = FollowService(session=session).get_follow_for_follower_and_following(follower_id=uuid.uuid4(), following_id=uuid.uuid4())
    assert follow is None
    session.close()
...
def test_create(before_create_normal_user):
    ...
    user1 = before_create_normal_user(session=session, user_dict=create_private_user())
    user2 = before_create_normal_user(session=session, user_dict=create_private_user())
    follow = FollowService(session=session).create(follow={"follower_id":user1.id, "following_id":user2.id, "status": StatusType.PENDING})
...
def test_update(before_create_normal_user, before_create_approved_follow_requests):
    ...
    follow = FollowService(session=session).update(follow={"follower_id":user2.id, "following_id":user1.id, "status": StatusType.PENDING}, db_follow=db_follow)
