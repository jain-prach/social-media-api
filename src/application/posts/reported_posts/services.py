...
class ReportPostAppService:
    ...
    def report_post(self, report:ReportPostData) -> Optional[ReportPost]:
        ...
        reported_by_list = [report.reported_by for report in post.report]
        if post and report.reported_by not in reported_by_list:
            return self.report_post_service.create_report(report=report)
        return None