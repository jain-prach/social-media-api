...
class TransactionService:
    ...
    def get_transaction_by_base_user_id(self, base_user_id: uuid.UUID) -> Optional[Transaction]:
        """get transaction by base_user_id from the database"""
        return self.db_session.scalars(
            select(Transaction).where(Transaction.base_user_id == base_user_id)
        ).first()

    def get_transaction_by_payment_id(
        payment_id: uuid.UUID, session: Session
    ) -> Transaction:
    ...
    def update(self, transaction, db_transaction: Transaction) -> Transaction:
