...
from .schemas import ReportPostSchema, ReportPostData
...
def report_post(current_user:AuthDep, post:ReportPostSchema, session:SessionDep):
    ...
    report = ReportPostData(reported_by=user.id, reason=post.reason, additional_text=post.additional_text, post_id=check_id(id=post.post_id))
